...
Die vollständige XML-Datei wurde im Verzeichnis des Windows-Systemvorbereitungstools (Sysprep, i.d.R. unter \verb|C:\Windows\System32\Sysprep|
) gespeichert. 
Anschließend erfolgte die Ausführung von Sysprep mit den Parametern \texttt{/oobe, /generalize, /shutdown sowie /unattend:(Pfad zur XML-Datei)}. 
...
Die Überprüfung der SIDs kann nach der VM-Erstellung über den Befehl \texttt{Get-LocalUser -Name Administrator | FL} erfolgen. 
...
Die Umsetzung basiert auf einfachem File Handling, wobei neue Verzeichnisse mit dem PowerShell-Befehl \texttt{New-Item} erzeugt werden.\\
...
Anschließend wurde die benötigte Rolle installiert und der Domain Controller mittels \texttt{Install-ADDSForest} heraufgestuft. 
...
Dies konnte mit einer dedizierten PowerShell-Funktion erreicht werden, die unter Verwendung von \texttt{Invoke-Command} den Domänenbeitritt in den VMs remote ausführt.\\
...
Die Benutzer wurden in Form von Hashmaps angelegt, während Gruppen über den Befehl \texttt{New-ADGroup} erstellt wurden. 
...
Die benötigten Registry-Keys lassen sich entweder durch das Auslesen bestehender Richtlinien über \texttt{Get-GPPrefRegistryValue} oder durch Dokumentationen im Internet identifizieren.\\
...
Die Freigabe konnte mit dem Befehl \texttt{New-SmbShare} umgesetzt werden, wobei mithilfe von Hashmaps und Schleifen Freigabename, Pfad und Berechtigungen automatisiert zugewiesen wurden.
Im Anschluss wurden die NTFS-Berechtigungen auf Verzeichnis- und Dateiebene konfiguriert.
Hierfür wurden die bestehenden Rechte eines Verzeichnisses zunächst mit \texttt{Get-Acl} in einer Variablen gespeichert, anschließend über eine Hashmap neue Einträge definiert (Benutzer bzw. Gruppe, Berechtigungsumfang sowie Anwendungsbereich: Ordner, Unterordner und Dateien).
Diese wurden in ein neues ACL-Objekt überführt und mit \texttt{Set-Acl} auf das jeweilige Verzeichnis angewendet.\\
...
\subsection{Testumgebung Server 2}
